...
const Analytics = () => {
  ...
  const { historicalShipping, alerts } = useShippingData(selectedPortId);
  ...
  const correlationData = historicalWeather.map((weatherItem, index) => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto py-6 px-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-maritime-800">Port Weather Analytics</h1>
            <p className="text-gray-500">Analyze historical weather patterns and shipping impacts</p>
          </div>
        ...
        {!selectedPortId ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-medium text-maritime-700">Select a port to view analytics</h2>
            <p className="text-gray-500 mt-2">Choose a port from the dropdown above to explore detailed analytics</p>
          </div>
        ) : (
          <Tabs defaultValue="weather" className="w-full">
                    ...
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Tooltip />
                          <Legend />
                          <Bar dataKey="windSpeed" name="Wind Speed (km/h)" fill="#0ea5e9" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </CardContent>
                    ...
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Tooltip />
                          <Legend />
                          <Bar dataKey="precipitation" name="Precipitation (mm)" fill="#8884d8" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </CardContent>
            ...
            <TabsContent value="shipping" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { title: 'Peak Traffic Times', value: '06:00 - 10:00', subtext: 'Highest vessel arrivals', trend: '+15% vs last month' },
                  { title: 'Average Processing Time', value: '28.4 hours', subtext: 'For all vessels', trend: '-5% vs last month' },
                  { title: 'Delay Frequency', value: '18%', subtext: 'Of vessels experience delays', trend: '+3% vs last month' }
                ].map((metric, index) => (
                  <Card key={index}>
                    <CardContent className="pt-6">
                      <h3 className="font-medium text-gray-500">{metric.title}</h3>
                      <p className="text-2xl font-bold text-maritime-700 mt-2">{metric.value}</p>
                      <p className="text-sm text-gray-500 mt-1">{metric.subtext}</p>
                      <p className={`text-xs mt-2 ${metric.trend.startsWith('+') ? 'text-red-500' : 'text-green-500'}`}>
                        {metric.trend}
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <CardContent>
                    <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
                      {alerts
                        ...
                        {alerts.filter(alert => alert.type === 'weather').length === 0 && (
                          <p className="text-center text-gray-500 py-4">No recent weather alerts</p>
                        )}
                    </div>
                  </CardContent>
                </Card>
